...
const Skills = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            const top = skillsContainer.offsetTop;
            const height = skillsContainer.offsetHeight;
            const scrollPosition = window.scrollY + window.innerHeight;

            if (scrollPosition > top && scrollPosition < top + height + 150) {
                setVisibleSkills(skills.map((_, index) => index));
            } else {
                setVisibleSkills([]); // Reset animation when scrolling away
            }
        };
